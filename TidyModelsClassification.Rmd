...
flavor of developers. There are several `R` Packages available in `CRAN` for supervised and 
unsupervised machine learning (ML). Most packages don't have unified interface, which, when 
present offers a consistent and easy-to-use machine learning workflow. **Tidy Modeling** approach which
consists of package ecosystem that supports unified workflow in ML including **Data Preprocessing**,
**Data Sampling**,**Model Building**, **Hyper Parameters Tuining**, and **Model Validation**. 

This tutorial provides a basic introduction to "Supervised Machine Learning" (SML)using **tiymodels**
packages. ML is a iterive process as modeling process (**Figure 1**).
...
EDA involves data general descriptive/inferential data analysis to capture main characteristics of data, often employing data visualization methods. EDA offers insight as to how the data should be manipulate to achieve the data analysis goal. 
...
Broadly speaking clustering and dimension reduction(e.g. PCA), although falls under EDA it is essentially part of
...
Different ML models have different parameters inherent their algorithms. Our data may best perform under different combinations of these parameters in specific ML model. MTS allows to find these combination either using **grid** search, **random** search or other optimization techniques. Automatic finding of such combinations of parameters often called as **"Hyperparameter Tuning"**.


<li> **Model Evaluation (ME)** :</li>
Finally, ME allows us to judge the performance of ML model. ME has several metrics depending on the nature of ML algorithms. For example, in Regression problem "RMSE: Root Mean Square Error", "MAE: Aean Absolute Error" etc can be used. Similarly, in classification
problem "confusion matrix based metrics can be used", e.g. Kappa statistics, Overall Accuracy, Mathew's correlation coefficient. Moreover, the ME involves generation of plots "Residual vs fitted", "feature importance" etc. 

In this tutorial, I will be using classification based problem in Land Use/Land Cover Classification of remotely sensed data employing tidymodel approach
</ol>
...
For ML we need some libraries, for now let's load `tidymodels` and `tidyverse`
```{r loadlibrary, echo= FALSE, message=FALSE, eval=TRUE}
...
I am using point `shapefile` of data based on training samples created in 
...
read your data from geodatabase. 

```{r loaddata, echo= FALSE, message=TRUE, eval=TRUE}
# load Data
...
We can now examine the data. Before that we can remove the spatial information, spatial data
handling process is slightly different from regular tabular data. To avoid such unwanted consequences, 
we can remove geometry and data.

```{r headData, echo=FALSE, message=FALSE, eval=TRUE, results= 'asis', fig.cap= " Plot of trainiing data", fig.align='center'}
# plot spatial data
...
```{r freqResponse, eval=TRUE, echo=FALSE}
...
Out of 41 variables two (MNDSI, and SNDSI) have constant values. Usually these type of variables are automatically removed from the model. However, we can remove unwanted variables. Let's get the vector of variable `names ()`
...
Now we have all data ready. But still Class_name 1, and 2 have fewer samples. Let's remove them and change the data type of "Class_name" into factor. 
...
Here, we will splint data into Training (80 %), and remaining data  (20%) will be classified as Testing data, where the former data set will be used to train the model as its name suggests. Test data will be used to evaluate model's performance.
...
# for cross validation of restuls using resampling
...
Now we have ready recipe for make a model. In this example, we will build the popular 
tree based machine learning model called 'Random Forest'. We need `parsnip` package for this task.
...
Lets use `rand_forest` (Model), and tune `m_try` (Argument), using `ranger` (engine), for our classification problem (land use land cover with six classes)
...
# Put Classification in Workflow
...
We selected the `mtry` parameter t to be tuned, we need to tune it before fitting our model. When we don’t have any parameters to tune, we can skip tuning process.
...
Multiple parameters can be tuned using `expand.grid()` function. In random forest number of trees, and mtry can be tuned together, see below
...
rf.params
```
we know know the best mtry = `r rf.params[[1,1]]`, and trees = `rf.params[[1,2]]`
...
```
# Evaluate the Model: Hold-out Test Data Set 
...
kbl(test.pred[1:10,1:10], caption =  "Performanc of RF on test data set",
    booktabs = TRUE)%>% 
  kable_styling(latex_options = c("striped", "hold_position"))
```
```{r confmat, eval=TRUE}
...
Following the usual modeling process we have fitted model using training data, and evaluated using testing data. Once the final model is determined, we can train the model using full data set and use it to predict the response to new data. Hoover, we usually have training data collected from on-screen digitization. In this case, our usual approach of dividing data into training, and testing and prediction to whole area is usually the norm. However, we can train final model on your full data set and then use it to predict the response for new data. In this case new data set could be data for whole study area. 
